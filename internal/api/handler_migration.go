...
// MigrationLoadRequest 加载数据库请求
type MigrationLoadRequest struct {
	DBPath string `json:"db_path"`
}

// MigrationTableRequest 查询表数据请求
...
// handleMigrationLoad 打开指定路径的 SQLite 数据库，返回表列表及记录数。
...
// handleMigrationTable 查询指定表的数据（分页）。
...
// handleMigrationFileList 列出目录中的文件（使用 velo fileserver）。
...
// handleMigrationCommonDirs 返回常用目录列表。
